...
The resulting checkpoints can be evaluated with:
  - evaluate_olora_t2m_moe_joined.py
  - evaluate_olora_m2t_moe_joined.py
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="O-LoRA MoE Joined Training")
    ...
    p.add_argument(
        ...
        type=int,
        default=128,
        help="T2M-only: max_length for caption-only tokenization when collecting router embeddings.",
    )
    ...
    if args.out_dir is None:
        args.out_dir = str(PROJECT_ROOT / "experiments" / "olora_moe_joined" / args.training_task / "v1")
...
def _extract_router_embedding(
    ...
    device: torch.device,
    *,
    router_text_max_length: int,
) -> torch.Tensor:
    """Compute routing embeddings.

    t2m: caption-only mean pooled LLM input embeddings (no prompt template).
    m2t: mean pooled motion-token embeddings.
    """
    if training_task == "t2m":
        # Joined variant: caption-only mean pooled LLM input embeddings (no prompt template).
        return compute_caption_router_embedding(
            model=model,
            captions=captions,
            device=device,
            ...
            torch.save(
                {
                    ...
                    "router_config": {
                        ...
                        # Extra metadata for T2M joined routing. Safe to ignore for M2T.
                        "router_embed_source": "caption_only",
                        "router_text_max_length": int(getattr(args, "router_text_max_length", 128)),
                    },
                },
                router_path,
