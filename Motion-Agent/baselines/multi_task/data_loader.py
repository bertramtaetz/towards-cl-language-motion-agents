...
class SingleTaskMotionDataset(data.Dataset):
    ...
    def __init__(
        ...
        if task_splits_path is None:
            # task_splits.json is in transfer_learning folder (shared by both baselines)
            task_splits_path = TASK_SPLITS
        if data_root is None:
            # HumanML3D is in datasets folder at project root
            data_root = DATA_ROOT
        ...
        # Load normalization stats (Motion-Agent/checkpoints/...)
...
class StratifiedBatchSampler(data.Sampler):
    """
    Sampler that ensures equal representation of all tasks in each batch.
    
    For multi-task learning, this prevents any single task from dominating
    gradient updates within a batch.
    """
    
    def __init__(self, task_to_indices: dict, batch_size: int, drop_last: bool = True):
        self.task_to_indices = {k: list(v) for k, v in task_to_indices.items()}
        self.num_tasks = len(task_to_indices)
        self.batch_size = batch_size
        self.drop_last = drop_last
        
        # Samples per task per batch (ensure at least 1)
        self.samples_per_task = max(1, batch_size // self.num_tasks)
        self.actual_batch_size = self.samples_per_task * self.num_tasks
        
        # Calculate number of complete batches
        min_task_size = min(len(indices) for indices in self.task_to_indices.values())
    ...
    def __iter__(self):
        ...
        for batch_idx in range(self.num_batches):
            ...
            for task_idx in range(self.num_tasks):
                ...
                batch.extend(shuffled[task_idx][start:end])
            
            # Shuffle within batch to prevent task ordering bias
...
def get_multitask_loader(
    ...
    Args:
        split: 'train', 'val', or 'test'
        batch_size: Batch size (will be adjusted for stratified sampling)
        w_vectorizer: Word vectorizer for text encoding
        unit_length: VQ-VAE temporal downsampling factor
        num_workers: Number of data loading workers
        stratified: If True, use stratified batch sampling for balanced tasks
                   (Note: disabled when distributed=True for DDP compatibility)
        task_splits_path: Path to task_splits.json
        distributed: If True, skip custom samplers for Accelerate compatibility
    ...
    # Note: Custom batch_sampler doesn't work well with Accelerate's DDP
    # When distributed=True, we use standard DataLoader and let Accelerate handle distribution
    if stratified and split == 'train' and not distributed:
        ...
        # Standard DataLoader - works with Accelerate DDP
        # The balanced dataset already has equal samples per task,
        # so random shuffling will approximately balance tasks per batch
        loader = data.DataLoader(
            dataset,
            batch_size=batch_size,
            shuffle=(split in ['train', 'trainval']),
